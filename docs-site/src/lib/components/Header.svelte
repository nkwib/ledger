<script>
  ...
  import { GITHUB_URL, VERSION } from '$lib/site.js';
  ...
  const docPrefixes = ['/tutorial', '/howto', '/reference', '/threat-model', '/design'];

  function isActive(path, currentPath) {
    if (path === '/tutorial') {
      // "Docs" stays lit across the whole docs group except the playground.
      return docPrefixes.some((p) => currentPath === p || currentPath.startsWith(p + '/'));
    }
